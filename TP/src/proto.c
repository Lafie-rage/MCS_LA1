		...
		strcpy(param2,"");
 		switch(req.reqNum){
 			case CMD_CONNECT_NUM :
				...
				user_t user = retrieveUserByName(users,param2);
				if (user.socket != -1){
				...
				if (users->size == MAX_USER){
					envoyerRequeteWithReqNum(socketDial, "CMD_ERROR_SERVER_FULL",CMD_ERROR_SERVER_FULL);
				}
				//ajoute l'user à la table
				if (addUser(users, param2, socketDial) == -1 ){
					envoyerRequeteWithReqNum(socketDial, "CMD_ERROR_UNKNOW_ERROR",CMD_ERROR_UNKNOW_ERROR); // logiquement impossible
					return;
				}
 			...
 			case CMD_LIST_NUM : 
 				...
 				envoyerRequeteWithReqNum(socketDial, stringListUser,CMD_LIST_NUM);
 				break;
