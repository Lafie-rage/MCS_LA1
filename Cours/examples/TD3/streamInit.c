...
int dialogSocket; // Socket d'écoute en mode stream
...
void installSigServ(int sigNum) {
  sturct sigaction newAction;
  newAction.sa_handler = deroute;
  newAction.sa_flags = SA_RESTART;
  sigemptyset(&newAction.sa_mask);
  CHECK(sigaction(sigNum, n&newAct, NULL), "SERVER : Problème application du sigaction");
}
